...
%打标签
for i=1:length(gene)
    index=find(strcmp(gene{i},cgc_gene)==1);
    if index
...
%load('BRCA_Dif_transcription.mat')
ex=Dif_transcription;
ex=ex/max(ex);
d=0.85;
pair_co=pair_co;
...
[a,b]=find(weight);
k=unique(a);
wx=weight*ones(length(gene),1);
wx=wx/max(wx);
wx=w*wx+e*ex;
r=Generank(adj_m,wx,d);
r_co=num2cell(r);
relt_co=[gene r_co];
...
pair_co(location,:)=[];
% mRNAgene与差异甲基化基因对应
Mex=zeros(length(gene),1);
for i=1:length(dmggene)
    index=find(strcmp(dmggene{i},gene));
    if index
    Mex(index)=Dif_methylation{i,2};
    end
end
ex=Mex;
ex=ex/max(ex);
...
[a,b]=find(weight);
k=unique(a);
wx=ones(length(gene),1);
ex=ones(length(gene),1);
for j = 1:size(k,1)
    i=k(j);
    locate=b(find(a==i));
    wx(i)=sum(weight(i,locate)'.*ex(locate));
end
r=Generank(adj_m,wx,d);
r_cmi=num2cell(r);
relt_cmi=[gene r_cmi];
...
d=0.85;
r=Generank(adj_new,wx,d);%得到generank值
r_sm=num2cell(r);
relt_sm=[gene r_sm];
